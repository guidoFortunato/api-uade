...
export const MovieCard = ({ title, image, description, movie }) => {
  ...
  const titleDescription = title.charAt().toUpperCase() + title.substring(1).toLowerCase();
  const pathTitle = title
    .replace(/[.:]/g, " ") // Reemplazar puntos y dos puntos por espacios
  ...
  return (
    <div className="rounded-lg relative hover:cursor-pointer mx-1">
      <img className="rounded-lg" src={image} alt={titleDescription} />
